# train_rawboost_meta_ensemble.py
import sys
import os
import numpy as np
import torch
...
ROOT_DIR = os.path.abspath(
    os.path.join(CURRENT_DIR, '..')) if 'ensemble' in CURRENT_DIR else CURRENT_DIR
...
OUTPUT_WEIGHTS = os.path.join(MODELS_DIR, "meta_ensemble_rawboost_best.pth")
...
class MetaLearner(nn.Module):
    def __init__(self, input_dim=616, hidden_dim=256,
...
def main():
    ...
    print(f"Initiating GPU-native RawBoost End-to-End Meta Ensemble Training on {device}")
    ...
    wrapper_model = EndToEndEnsemble(
        aasist_model, resnet_model, meta_learner
    ).to(device)
    ...
    optimizer = optim.AdamW(
        wrapper_model.parameters(), lr=LR, weight_decay=1e-4
    )
    ...
    for epoch in range(TOTAL_EPOCHS):
        ...
        for waveforms, labels in pbar:
            ...
            mel = mel_transform(aug_waveforms)
            mel_db = amp_to_db(mel).unsqueeze(1)

            out_meta, out_a, out_r = wrapper_model(
                aug_waveforms, mel_db, return_base_outs=True
            )
            ...
            torch.nn.utils.clip_grad_norm_(
                wrapper_model.parameters(), max_norm=1.0
            )
        ...
        with torch.no_grad():
            for wv, lv in tqdm(
                ...
                pc.extend(
                    torch.softmax(out_meta, dim=1)[:, 1].cpu().numpy()
                )
        ...
        if epochs_no_improve >= 20:
            print(
                ...
                "GPU-native RawBoost Meta Ensemble has converged."
            )
            break
