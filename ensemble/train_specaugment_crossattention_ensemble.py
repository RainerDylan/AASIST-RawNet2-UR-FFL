# train_rawboost_crossattention_ensemble.py
import sys
import os
import time
import datetime
...
OUTPUT_WEIGHTS = os.path.join(MODELS_DIR, "crossattention_ensemble_rawboost_best.pth")
...
# ── RawBoost — fully GPU-native ────────────────────────────────────────────────
# Tak et al. (2022) RawBoost: A Simple and Effective Data Augmentation Method
# for Automatic Speaker Verification Anti-Spoofing. ICASSP 2022.
# All three components (LnL, ISD, SSI) execute entirely on the input device.
# No CPU round-trip, no Python loops over batch elements.
RAWBOOST_ALPHA = 0.05


def apply_rawboost_gpu(waveform: torch.Tensor,
                       alpha: float = 0.05) -> torch.Tensor:
    """
    GPU-native RawBoost: LnL + ISD + SSI applied to a batch of waveforms.

    waveform : (B, L) float32 tensor — must already be on target device.
    alpha    : blend coefficient; output = (1-alpha)*clean + alpha*degraded.

    LnL  — random FIR convolution simulating room/channel effects.
    ISD  — impulsive signal-dependent noise simulating transient artefacts.
    SSI  — stationary AWGN at a random SNR in [10, 40] dB.
    """
    B, L = waveform.shape
    device = waveform.device

    norms = waveform.abs().max(dim=1, keepdim=True)[0].clamp(min=1e-9)
    x_n   = waveform / norms

    # LnL: Random FIR convolution
    N_f = 5
    b   = torch.cat([torch.ones(B, 1, device=device),
                     torch.rand(B, N_f - 1, device=device) * 2.0 - 1.0],
                    dim=1)
    pad      = N_f // 2
    x_padded = F.pad(x_n, (pad, pad))
    windows  = x_padded.unfold(1, N_f, 1)
    x_lnl    = (windows * b.unsqueeze(1)).sum(dim=-1)

    # ISD: Impulsive signal-dependent noise
    P    = 10.0
    g_sd = 0.5
    beta = torch.rand(B, 1, device=device) * P / 100.0
    z    = torch.randn(B, L, device=device) * g_sd * x_lnl.abs()
    mask = (torch.rand(B, L, device=device) < beta).float()
    x_isd = x_lnl + z * mask

    # SSI: Stationary AWGN
    snr_db  = torch.rand(B, 1, device=device) * 30.0 + 10.0
    snr_lin = 10.0 ** (snr_db / 10.0)
    p_sig   = (x_isd ** 2).mean(dim=1, keepdim=True).clamp(min=1e-9)
    p_noise = p_sig / snr_lin
    noise   = torch.randn(B, L, device=device) * p_noise.sqrt()
    x_ssi   = (x_isd + noise).clamp(-1.0, 1.0)

    x_aug = x_ssi * norms
    return (1.0 - alpha) * waveform + alpha * x_aug
...
class EndToEndEnsemble(nn.Module):
    def __init__(self, aasist, resnet, fuser):
        ...
        self._ea = [None]
        self._er = [None]
        self._ha = aasist.fc.register_forward_hook(
            lambda m, i, o: self._ea.__setitem__(0, i[0])
        )
        self._hr = resnet.fc.register_forward_hook(
            lambda m, i, o: self._er.__setitem__(0, i[0])
        )
    ...
    for n, mod in m.named_modules():
        ...
        if isinstance(mod, (nn.Conv1d, nn.Conv2d)):
            (nn.init.xavier_uniform_ if ia else
             lambda w: nn.init.kaiming_normal_(
                 w, mode="fan_out", nonlinearity="relu"))(mod.weight)
            if mod.bias is not None:
                nn.init.zeros_(mod.bias)
        elif isinstance(mod, nn.Linear):
            (nn.init.xavier_uniform_ if ia else
             lambda w: nn.init.kaiming_normal_(
                 w, mode="fan_in", nonlinearity="relu"))(mod.weight)
            if mod.bias is not None:
                nn.init.zeros_(mod.bias)
...
def create_train_sampler(dataset):
    ...
    return WeightedRandomSampler(
        [w[l] for l in labels], len(labels), replacement=True
    )
...
def create_balanced_val_indices(dataset):
    ...
    idx = np.concatenate([
        rng.choice(gen_idx, n, replace=False),
...
def main():
    ...
    print(f"GPU-native RawBoost Cross-Attention Ensemble — {device}")
    print("=" * 70)
    ...
    optimizer = optim.AdamW([
        {'params': model.aasist.parameters(),
    ...
    print(f"RawBoost (GPU-native LnL+ISD+SSI): alpha={RAWBOOST_ALPHA}")
    print(f"Applied to: waveform (AASIST) + augmented mel (ResNet), "
          f"train only, all on {device}")
    print(f"Loss: META_W={META_W}  AUX_W={AUX_W}")
    print("=" * 70)

    best_eer, no_imp = float("inf"), 0
    t0 = time.time()
    H = {k: [] for k in [
        "tr_loss", "vl_loss", "l_meta", "l_aux",
    ...
    for ep in range(TOTAL_EPOCHS):
        ...
        for wav, lbl in bar:
            ...
            # GPU-native RawBoost: same augmented waveform for both backbones
            aug_wav = apply_rawboost_gpu(wav, alpha=RAWBOOST_ALPHA)

            # AASIST receives augmented waveform
            # ResNet receives mel of augmented waveform
            mel = a2db(mel_t(aug_wav)).unsqueeze(1)

            optimizer.zero_grad(set_to_none=True)
            om, oa, or_ = model(aug_wav, mel, return_base=True)
            omf = om.float()
            oaf = oa.float()
            orf = or_.float()
        ...
        H["tr_loss"].append(st / nb)
        H["l_meta"].append(sm / nb)
        H["l_aux"].append(sa / nb)
        ...
        with torch.no_grad():
            for wav, lbl in tqdm(
                val_loader, desc=f"Ep {ep+1} [Val]", leave=False
            ):
                wav = wav.squeeze(1).to(device)
                lbl = lbl.to(device)
                # No RawBoost during validation — clean inputs only
        ...
        H["vl_eer"].append(veer)
        H["vl_auc"].append(vauc)
        H["vl_mdc"].append(vmdc)
    ...
    ax[0].plot(E, H["vl_loss"], label="Val Loss (balanced)",
               color="red", ls="--")
    ax[0].plot(E, H["l_meta"], label="L_meta (fusion)",
               color="steelblue", ls=":")
    ax[0].plot(E, H["l_aux"], label="L_aux (base models)",
               color="darkorange", ls=":")
    ax[0].set_title("Loss Trajectory (RawBoost GPU)")
    ax[0].set_xlabel("Epoch")
    ax[0].legend(fontsize=8)
    ax[0].grid(True, ls=":", alpha=0.6)

    ax[1].plot(E, H["tr_eer"], label="Train EER %", color="teal")
    ax[1].plot(E, H["vl_eer"], label="Val EER % (balanced)",
               color="purple", ls="--")
    ...
    info = (
        ...
        "Augment   : GPU-native RawBoost (LnL+ISD+SSI)\n"
        f"  alpha            = {RAWBOOST_ALPHA}\n"
        "  Device           : GPU (no CPU round-trip)\n"
    ...
    ax[5].text(
        0.05, 0.95, info, transform=ax[5].transAxes, fontsize=10,
        va="top", family="monospace",
        bbox=dict(boxstyle="round", facecolor="lightyellow", alpha=0.85)
    )

    fig.suptitle(
        "GPU-native RawBoost Cross-Attention Ensemble — Cold-Start\n"
        "(Balanced Val Subset · Differential LR · GPU RawBoost Only)",
        fontsize=13
    )
    fig.tight_layout()
    gp = os.path.join(
        RESULTS_DIR, "crossattention_ensemble_rawboost_metrics.png"
    )
