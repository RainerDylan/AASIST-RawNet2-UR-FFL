...
def spec_augment(mel_db: torch.Tensor,
    ...
    """
    SpecAugment (Park et al. 2019) applied to mel spectrogram features.
    mel_db: (B, 1, n_mels, T)
    freq_mask_param: maximum number of mel frequency channels to mask.
    time_mask_param: maximum number of time steps to mask.
    num_freq_masks: number of frequency masks applied per sample.
    num_time_masks: number of time masks applied per sample.
    Masked regions are set to zero (mean value for normalised features).
    """
    out = mel_db.clone()
    B, C, F, TT = out.shape
    for b in range(B):
...
def main():
    ...
    print(f"Initiating SpecAugment ResNet-SimAM Training on {device}")
    ...
    print(f"Applied to: mel spectrogram (n_fft=512, hop=160, n_mels=80)")
    print("=" * 60)
    ...
    for epoch in range(total_epochs):
        ...
        for waveforms, labels in pbar:
            ...
            # Apply SpecAugment to mel features (training only)
        ...
        if val_eer < best_eer:
            best_eer = val_eer
            epochs_no_improve = 0
            save_path = os.path.join(MODELS_DIR, "resnet_specaugment_best.pth")
    ...
    print("SpecAugment ResNet Training complete. Generating learning curve graphs...")
    ...
    plt.tight_layout()
    graph_path = os.path.join(RESULTS_DIR, "resnet_specaugment_metrics.png")
